...
.render_children <- function(node, .arg) {
  ...
  for (i in seq_along(children)) {
    if (!is_leaf[i]) {
      # Compound child: keep the node so render_block() can render it with the
      # correct absolute indent and width (needed for wrapping).
      entries <- c(entries, list(list(role = role_of(i), node = children[[i]])))
    }
      ...
      entries <- c(entries, list(list(role = role_of(i), text = text)))
    }
      ...
      merged <- .render_prefix_group(templates[[as.character(group_id[i])]],
                                     members, and_or, env)
      entries <- c(entries, list(list(role = role_of(i), text = merged)))
...
# Display width consumed by a bullet marker (styled glyph + trailing space), or 0
# for a header with no marker.
.marker_width <- function(marker) {
  if (nzchar(marker)) cli::ansi_nchar(marker) + 1L else 0L
}

# The width cli wraps a condition message to: `cli.condition_width` (testthat pins
# it to Inf so test output is reproducible), then `cli.width`, then whatever cli
# infers for the console. Reading the same source keeps a cascade in step with the
# single-line messages err() hands to cli.
.cnd_width <- function() {
  width <- getOption("cli.condition_width", getOption("cli.width"))

  if (is.null(width)) cli::console_width() else width
}

# Join a sentence prefix (e.g. "When `x` is supplied,") to a message, lowercasing
# the message's first letter so the two read as one sentence. Done before wrapping
# so the prefix is part of the width budget.
.add_prefix <- function(text, prefix) {
  if (!nzchar(prefix)) {
    return(text)
  }

  paste(prefix, ansi_lower_first(text))
}

# Render one display line -- leading `indent` spaces, an optional styled marker
# ("glyph "), then `text` wrapped to `width` with a hang indent so continuation
# lines align under the text. `width = Inf` disables wrapping (the layout is then
# byte-identical to a single un-wrapped line).
.wrap_dline <- function(indent, marker, text, width) {
  prefix <- paste0(strrep(" ", indent),
                   if (nzchar(marker)) paste0(marker, " ") else "")

  if (is.infinite(width)) {
    return(paste0(prefix, text))
  }

  prefix_w <- indent + .marker_width(marker)
  pieces <- cli::ansi_strwrap(text, width = max(width - prefix_w, min(20L, width)))

  c(paste0(prefix, pieces[1L]),
    if (length(pieces) > 1L) paste0(strrep(" ", prefix_w), pieces[-1L]))
}

# Wrap the text that becomes the very first physical line, matching how rlang and
# cli wrap the main message of a cli-formatted error: cli::format_error() prepends
# "Error: " before wrapping (reserving room for the prefix R prints ahead of a
# short message) and strips it again afterwards, rlang prefixes that line with
# "! ", and its continuation lines are indented underneath. A cascade is emitted
# verbatim, so both the reservation and the hang indent are applied here.
.wrap_header <- function(text, width) {
  if (is.infinite(width)) {
    return(text)
  }

  hang <- .marker_width("!")
  box <- max(width - hang, min(20L, width))

  pieces <- cli::ansi_strwrap(paste0("Error: ", text), width = box)

  # At a very narrow width the reservation lands on the first line on its own, and
  # without its trailing space -- hence the optional match, and the re-wrap that
  # drops the reservation rather than leaving the first line blank.
  pieces[1L] <- sub("^Error: ?", "", pieces[1L])

  if (!nzchar(pieces[1L])) {
    pieces <- cli::ansi_strwrap(text, width = box)
  }

  c(pieces[1L],
    if (length(pieces) > 1L) paste0(strrep(" ", hang), pieces[-1L]))
}

# Render a node to a vector of physical lines. Compound children that failed are
# expanded as indented sub-blocks (each block's header takes the parent's glyph,
# its own children indented under the header text); passing/leaf children are
# single lines. Lines longer than `width` wrap with a hang indent. `indent` is the
# absolute column of this node's header/bullets, `marker` is the styled glyph for
# its header (empty at the top level), and `prefix` is an optional sentence prefix
# for the top-level header (see .add_prefix()).
render_block <- function(node, .arg, width = Inf, indent = 0L, marker = "",
                         prefix = "") {
  if (identical(node[["kind"]], "leaf")) {
    return(.wrap_dline(indent, marker, .add_prefix(node[["msg"]], prefix), width))
  }

  entries <- .render_children(node, .arg)

  # A lone entry needs no header: the node "becomes" that entry.
  if (length(entries) == 1L) {
    e <- entries[[1L]]

    if (is_not_null(e[["node"]])) {
      return(render_block(e[["node"]], .arg, width, indent, marker, prefix))
    }

    # A lone leaf at the top (no marker) is returned raw so .throw_arg_node()
    # routes it through err() (which wraps single-line messages via cli).
    if (!nzchar(marker)) {
      return(.add_prefix(e[["text"]], prefix))
    }

    return(.wrap_dline(indent, marker, e[["text"]], width))
  }
  ...
  # The top-level header becomes the first physical line, so it wraps like the main
  # message of a cli-formatted error; a nested header is a bullet like any other.
  out <- if (indent == 0L && !nzchar(marker)) {
    .wrap_header(.add_prefix(header, prefix), width)
  }
  else {
    .wrap_dline(indent, marker, header, width)
  }

  child_indent <- indent + .marker_width(marker)

  for (e in entries) {
    g <- glyphs[[e[["role"]]]]

    if (is_not_null(e[["node"]])) {
      out <- c(out, render_block(e[["node"]], .arg, width, child_indent, g))
    }
    else {
      out <- c(out, .wrap_dline(child_indent, g, e[["text"]], width))
    }
  }
...
.throw_arg_node <- function(node, .arg, .call, .frame) {
  ...
  lines <- render_block(node, .arg, width = .cnd_width())
