...
test_that("arg_and() marks passed checks with a tick and failed checks with a cross", {
  rlang::local_options(cli.unicode = TRUE, cli.num_colors = 1, width = 200)
...
test_that("deeply nested and/or renders as a clean cascade under non-UTF-8 output (regression)", {
  ...
  rlang::local_options(cli.unicode = FALSE, cli.num_colors = 1, width = 200)
...
test_that("bullet glyphs match a native cli condition's styling (theme-aware, not hard-coded)", {
  ...
  rlang::local_options(cli.unicode = TRUE, cli.num_colors = 256, width = 200)
...
test_that("cascade lines wrap at the console width with a hang indent", {
  # cli.condition_width is the width cli wraps condition messages to, and the one
  # testthat pins to Inf for reproducible output, so it is what has to be set here.
  rlang::local_options(width = 60, cli.condition_width = 60,
                       cli.num_colors = 1, cli.unicode = TRUE)

  opts <- c("alpha", "bravo", "charlie", "delta", "echo", "foxtrot")
  g <- function(z) arg_and(z, arg_number, arg_or(arg_flag, arg_element(values = opts)))
  msg <- conditionMessage(rlang::catch_cnd(g("x")))
  lines <- strsplit(msg, "\n", fixed = TRUE)[[1L]]

  # Every physical line fits within the width.
  expect_true(all(cli::ansi_nchar(lines) <= 60L))
  # The long alternative wrapped onto multiple lines...
  expect_gt(length(lines), 3L)
  # ...with a 2-space hang indent aligning continuations under the bullet text.
  expect_true(any(startsWith(lines, "  ")))
  # Structure is intact.
  expect_identical(lines[1L], "All of the following conditions must be met:")
  expect_true(any(grepl("must be a number", lines, fixed = TRUE)))
})

test_that("a wide width leaves cascade lines unwrapped", {
  rlang::local_options(width = 200, cli.condition_width = 200,
                       cli.num_colors = 1, cli.unicode = TRUE)
  ...
  # The long alternative stays on one unbroken line (no hang-indented wrapping).
  expect_match(msg,
               'or one of "alpha", "bravo", "charlie", "delta", "echo", or "foxtrot"',
               fixed = TRUE)
  expect_false(grepl("\n  ", msg, fixed = TRUE))
})

test_that("rendering degrades gracefully at a very narrow width", {
  rlang::local_options(width = 20, cli.condition_width = 20,
                       cli.num_colors = 1, cli.unicode = TRUE)

  # At an extreme width the wrapper must not error; every physical line still
  # respects a sensible minimum wrap width.
  opts <- c("alpha", "bravo", "charlie")
  g <- function(z) arg_and(z, arg_number, arg_or(arg_flag, arg_element(values = opts)))
  cnd <- rlang::catch_cnd(g("x"))
  expect_s3_class(cnd, "error")
  expect_gt(length(strsplit(conditionMessage(cnd), "\n", fixed = TRUE)[[1L]]), 1L)
})

test_that("the wrapping helpers fall back sensibly", {
  # With neither cli width option set, the width is the one cli infers itself.
  rlang::local_options(cli.condition_width = NULL, cli.width = NULL, width = 72)
  expect_identical(.cnd_width(), cli::console_width())

  # Too narrow to hold cli's reserved "Error: " prefix alongside the first word:
  # the header is wrapped without the reservation rather than keeping a stray
  # prefix or a blank first line.
  lines <- .wrap_header("supercalifragilisticexpialidocious and then some", 20L)
  expect_identical(lines, c("supercalifragilisticexpialidocious", "  and then some"))
})

test_that("a cascade wraps exactly like the equivalent cli-formatted condition", {
  rlang::local_options(width = 80, cli.condition_width = 80,
                       cli.num_colors = 1, cli.unicode = TRUE)

  header <- "When `vcov` is not NULL, at least one of the following conditions must be met:"
  bullets <- c(
  ...
  width <- .cnd_width()
  expect_identical(width, 80)

  glyph <- .themed_glyph("*")
  ours <- c(.wrap_header(header, width),
            .wrap_dline(0L, glyph, bullets[1L], width),
            .wrap_dline(0L, glyph, bullets[2L], width))

  # What rlang and cli produce for the same header and bullets, as printed.
  cnd <- rlang::catch_cnd(rlang::abort(c(header, "*" = bullets[1L], "*" = bullets[2L]),
                                      use_cli_format = TRUE))
  theirs <- rlang::cnd_message(cnd, prefix = TRUE) |>
    strsplit("\n", fixed = TRUE) |>
    unlist(recursive = FALSE)

  theirs <- theirs[-1L]                     # the "Error:" line
  theirs[1L] <- sub("! ", "", theirs[1L], fixed = TRUE)

  expect_identical(ours, theirs)
})

test_that("when_supplied()/when_not_null() wrap their prefix into the header", {
  rlang::local_options(width = 60, cli.condition_width = 60,
                       cli.num_colors = 1, cli.unicode = TRUE)

  f <- function(z) when_supplied(z, arg_or(arg_and(arg_all_NA, arg_length(1L)),
                                           arg_and(arg_no_NA, arg_length(2L))))
  lines <- conditionMessage(rlang::catch_cnd(f(1))) |>
    strsplit("\n", fixed = TRUE) |>
    unlist(recursive = FALSE)

  # The prefix joins the header before it is wrapped, so the header wraps to the
  # width instead of being pushed past it, and its continuation is hang-indented
  # under the "! " rlang prepends to the first line.
  expect_true(all(cli::ansi_nchar(lines) <= 60L))
  expect_lte(cli::ansi_nchar(lines[1L]) + 2L, 60L - 7L)
  expect_true(startsWith(lines[2L], "  "))
  expect_identical(paste(cli::ansi_trimws(lines[1:2]), collapse = " "),
                   "When `z` is supplied, at least one of the following conditions must be met:")

  # The bullets below the header are untouched by the prefix.
  expect_true(any(grepl("^• All of the following", lines)))
})
